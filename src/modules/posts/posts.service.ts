...
export const updatePost = async (postId: string, body: Partial<IPost>, userId: Types.ObjectId) => {
    const post = await Post.findById(postId);
    if (!post) {
    ...
    return await Post.findByIdAndUpdate(postId, { content: body.content }, { new: true });
};
